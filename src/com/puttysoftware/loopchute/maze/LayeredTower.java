...
class LayeredTower implements Cloneable {
	...
	for (u = 0; u < y; u++) {
		...
		for (w = 0; w < z; w++) {
			...
			try {
			    tempStorage.setCell(this.getCell(v, u, w, e), u, v, w, e);
	...
	do {
	    ...
	    try {
		final MazeObject obj = this.getCell(fx1, fy1, zLoc, MazeConstants.LAYER_OBJECT);
	...
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		for (z = 0; z < this.getFloors(); z++) {
		    final MazeObject obj = this.getCell(y, x, z, MazeConstants.LAYER_OBJECT);
    ...
    public void fillFloorAndLayer(final MazeObject fillWith, final int z, final int e) {
	int x, y;
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		this.setCell(fillWith, y, x, z, e);
    ...
    public void fillLayerRandomlyCustom(final Maze maze, final int w, final int e) {
	for (int z = 0; z < this.getFloors(); z++) {
	    this.fillFloorAndLayerRandomlyCustom(maze, z, w, e);
	}
    }

    public void fillFloorAndLayerRandomly(final Maze maze, final int z, final int w, final int layer) {
	// Pre-Pass
	final MazeObjectList objects = LoopChute.getApplication().getObjects();
	final MazeObject pass1Fill = PreferencesManager.getEditorDefaultFill(layer);
	RandomRange r = null;
	int x, y;
	// Pass 1
	...
	final int columns = this.getColumns();
	final int rows = this.getRows();
	final MazeObject[] objectsWithoutPrerequisites = objects.getAllWithoutPrerequisiteAndNotRequired(layer);
	if (objectsWithoutPrerequisites != null) {
	    r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
	    for (x = 0; x < columns; x++) {
		for (y = 0; y < rows; y++) {
		    final MazeObject placeObj = objectsWithoutPrerequisites[r.generate()];
		    final boolean okay = placeObj.shouldGenerateObject(maze, x, y, z, w, layer);
		    if (okay) {
			this.setCell(objects.getNewInstanceByName(placeObj.getName()), y, x, z, layer);
			placeObj.editorGenerateHook(y, x, z);
		    }
		}
	    }
    ...
    public void fillFloorAndLayerRandomlyCustom(final Maze maze, final int z, final int w, final int layer) {
	// Pre-Pass
	final MazeObjectList objects = LoopChute.getApplication().getObjects();
	final MazeObject pass1Fill = PreferencesManager.getEditorDefaultFill(layer);
	final MazeObject[] withoutRuleSets = objects.getAllObjectsWithoutRuleSets();
	...
	if (withoutRuleSets != null) {
	    final MazeObject[] objectsWithoutPrerequisites = MazeObjectList
		    .getAllWithoutPrerequisiteAndNotRequiredSubset(withoutRuleSets, layer);
	    if (objectsWithoutPrerequisites != null && objectsWithoutPrerequisites.length > 0) {
		r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
			final MazeObject placeObj = objectsWithoutPrerequisites[r.generate()];
			final boolean okay = placeObj.shouldGenerateObject(maze, y, x, z, w, layer);
			if (okay) {
			    this.setCell(objects.getNewInstanceByName(placeObj.getName()), y, x, z, layer);
			    placeObj.editorGenerateHook(y, x, z);
			}
	...
	if (withRuleSets != null) {
	    // Pass 4
	    final MazeObject[] objectsWithoutPrerequisites = MazeObjectList
		    .getAllWithoutPrerequisiteAndNotRequiredSubset(withRuleSets, layer);
	    if (objectsWithoutPrerequisites != null) {
		r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
			final MazeObject placeObj = objectsWithoutPrerequisites[r.generate()];
			final boolean okay = placeObj.getRuleSet().shouldGenerateObject(maze, y, x, z, w, layer);
			if (okay) {
			    this.setCell(objects.getNewInstanceByName(placeObj.getName()), y, x, z, layer);
			    placeObj.editorGenerateHook(y, x, z);
			}
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			lt.setCell(LoopChute.getApplication().getObjects().readMazeObject(reader, ver), y, x, z, e);
