...
public class MazeManager implements OpenFilesHandler {
	...
	if (saved) {
		...
		if (ff.getDescription().equals(xmf.getDescription())) {
		    PreferencesManager.setLastFilterUsedOpen(PreferencesManager.FILTER_MAZE);
		} else {
		    PreferencesManager.setLastFilterUsedOpen(PreferencesManager.FILTER_GAME);
		}
	...
	if (app.getMode() == Application.STATUS_GAME) {
		...
		if (extension != null) {
		    ...
		    this.lastUsedGameFile += Extension.getSavedGameExtensionWithPeriod();
		}
	...
	while (!FilenameChecker.isFilenameOK(fileOnly)) {
		...
		if (!FilenameChecker.isFilenameOK(fileOnly)) {
			...
			if (extension != null) {
			    ...
			    filename += Extension.getSavedGameExtensionWithPeriod();
			}
			this.lastUsedGameFile = filename;
