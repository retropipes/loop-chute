...
public class Maze implements MazeConstants {
	...
	this.basePath = System.getProperty("java.io.tmpdir") + File.separator + "loopchute" + File.separator + randomID
		+ ".maze";
	final File base = new File(this.basePath);
	final boolean success = base.mkdirs();
	if (!success) {
	    CommonDialogs.showErrorDialog("Maze temporary folder creation failed!", "loopchute");
	}
    }

    // Static methods
    public static String getMazeTempFolder() {
	return System.getProperty("java.io.tmpdir") + File.separator + "loopchute";
    }
	...
	if (this.levelCount > 1) {
		...
		final boolean delSuccess = this.getLevelFile(this.activeLevel).delete();
		if (!delSuccess) {
		    return false;
		}
	...
	final MazeObject fillWith = PreferencesManager.getEditorDefaultFill(layer);
	...
	this.mazeData.fillFloorAndLayerRandomly(this, z, this.activeLevel, layer);
    }
	...
	this.mazeData.fillFloorAndLayerRandomlyCustom(this, z, this.activeLevel, layer);
    }
