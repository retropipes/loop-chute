...
public class ImageCache {
    ...
    // Methods
    static BufferedImageIcon getCachedImage(final String name, final String baseName, final int transformColor,
	    final String attrName, final int attrColor) {
	if (!ImageCache.isInCache(name)) {
	    if (attrName != null && !attrName.isEmpty()) {
		BufferedImageIcon bii1 = GraphicsManager.getUncachedImage(baseName);
		bii1 = GraphicsManager.templateTransformImage(bii1, transformColor);
		BufferedImageIcon bii2 = GraphicsManager.getUncachedImage(attrName);
		bii2 = GraphicsManager.templateTransformImage(bii2, attrColor);
		final BufferedImageIcon bii3 = GraphicsManager.getCompositeImage(bii1, bii2);
		ImageCache.addToCache(name, bii3);
	    } else {
		BufferedImageIcon bii = GraphicsManager.getUncachedImage(baseName);
		bii = GraphicsManager.templateTransformImage(bii, transformColor);
