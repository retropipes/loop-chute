...
public class BreakableWallVertical extends GenericWall {
	...
	final int dirZ = LoopChute.getApplication().getMazeManager().getMaze().getPlayerLocationZ();
	...
	// Break up
	LoopChute.getApplication().getGameManager().morph(new Empty(), dirX, dirY, dirZ, MazeConstants.LAYER_OBJECT);
	final Maze m = LoopChute.getApplication().getMazeManager().getMaze();
	final MazeObject above = m.getCell(dirX, dirY - 1, dirZ, MazeConstants.LAYER_OBJECT);
	try {
	...
	try {
	    final MazeObject below = m.getCell(dirX, dirY + 1, dirZ, MazeConstants.LAYER_OBJECT);
