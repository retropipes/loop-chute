...
public class GameLogicManager {
	...
	try {
	    m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ() - 1,
	...
	final Maze m = app.getMazeManager().getMaze();
	try {
	    m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), floor, MazeConstants.LAYER_OBJECT);
	    return true;
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    return false;
	} catch (final NullPointerException np) {
	...
	do {
		...
		try {
		    if (this.checkSolid(this.savedMazeObject, below, nextBelow, nextAbove)) {
			redrawsSuspended = this.updatePositionRelativePull(dirX, dirY);
		    } else {
			redrawsSuspended = this.updatePositionRelativePush(dirX, dirY);
		    }
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    this.vwMgr.restoreViewingWindow();
		    m.restorePlayerLocation();
		    // Move failed - attempted to go outside the maze
		    o.moveFailedAction(false, m.getPlayerLocationX(), m.getPlayerLocationY(), this.objectInv);
	...
	if (acted.isPushable()) {
	    ...
	    this.fireMoveFailedActions(px + fX, py + fY, this.savedMazeObject, below, nextBelow, nextAbove);
	    this.fireStepActions();
	}
	...
	if (acted.isPullable() && this.isPullInProgress()) {
	    if (!this.checkPull(fX, fY, pullX, pullY, acted, previousBelow, below, this.savedMazeObject)) {
	...
	final Application app = LoopChute.getApplication();
	final Maze m = app.getMazeManager().getMaze();
	m.setCell(backUpObject, m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
		MazeConstants.LAYER_OBJECT);
	this.vwMgr.restoreViewingWindow();
	m.restorePlayerLocation();
    ...
    private boolean checkLoopCondition(final boolean proceed, final MazeObject groundInto, final MazeObject below,
	...
	if (this.objectInv.isItemThere(new SlipperyBoots())) {
	    return proceed && this.checkSolid(this.savedMazeObject, below, nextBelow, nextAbove);
	} else {
	    return proceed && !groundInto.hasFrictionConditionally(this.objectInv, false)
		    && this.checkSolid(this.savedMazeObject, below, nextBelow, nextAbove);
	}
    }

    private boolean checkSolid(final MazeObject inside, final MazeObject below, final MazeObject nextBelow,
	...
	MazeObject there = mm.getMazeObject(m.getPlayerLocationX() + cumX, m.getPlayerLocationY() + cumY,
	...
	if (there != null) {
		...
		cumPushX += xInc;
		cumPushY += yInc;
		there = mm.getMazeObject(m.getPlayerLocationX() + cumX, m.getPlayerLocationY() + cumY,
			m.getPlayerLocationZ(), MazeConstants.LAYER_GROUND);
		if (there == null) {
		    break;
		}
	...
	final Maze m = app.getMazeManager().getMaze();
	try {
	    m.setCell(o.getSavedObject(), m.getPlayerLocationX() + x, m.getPlayerLocationY() + y,
		    m.getPlayerLocationZ(), MazeConstants.LAYER_OBJECT);
	    m.setCell(o, m.getPlayerLocationX() + pushX, m.getPlayerLocationY() + pushY, m.getPlayerLocationZ(),
		    MazeConstants.LAYER_OBJECT);
	    if (g.overridesDefaultPostMove()) {
		g.postMoveAction(false, m.getPlayerLocationX(), m.getPlayerLocationY(), this.objectInv);
	    }
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	}
	...
	try {
	    m.setCell(o.getSavedObject(), m.getPlayerLocationX() - x, m.getPlayerLocationY() - y,
		    m.getPlayerLocationZ(), MazeConstants.LAYER_OBJECT);
	    m.setCell(o, m.getPlayerLocationX() - pullX, m.getPlayerLocationY() - pullY, m.getPlayerLocationZ(),
		    MazeConstants.LAYER_OBJECT);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	}
	...
	try {
	    final Application app = LoopChute.getApplication();
	    final Maze m = app.getMazeManager().getMaze();
	    final MazeObject below = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
		    MazeConstants.LAYER_GROUND);
	    final MazeObject nextBelow = m.getCell(m.getPlayerLocationX() + x, m.getPlayerLocationY() + y,
		    m.getPlayerLocationZ(), MazeConstants.LAYER_GROUND);
	    final MazeObject nextAbove = m.getCell(m.getPlayerLocationX() + x, m.getPlayerLocationY() + y,
		    m.getPlayerLocationZ(), MazeConstants.LAYER_OBJECT);
	    return this.checkSolid(this.savedMazeObject, below, nextBelow, nextAbove);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    return false;
	}
    }
	...
	try {
	    if (!m.getCell(x, y, z, MazeConstants.LAYER_OBJECT).isConditionallySolid(this.objectInv)) {
		m.setCell(this.savedMazeObject, m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
			MazeConstants.LAYER_OBJECT);
		m.setPlayerLocationX(x);
		m.setPlayerLocationY(y);
		m.setPlayerLocationZ(z);
		this.vwMgr.setViewingWindowLocationX(m.getPlayerLocationY() - this.vwMgr.getOffsetFactorX());
		this.vwMgr.setViewingWindowLocationY(m.getPlayerLocationX() - this.vwMgr.getOffsetFactorY());
		this.savedMazeObject = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
			MazeConstants.LAYER_OBJECT);
		app.getMazeManager().setDirty(true);
    ...
    public void redrawOneSquare(final int x, final int y, final boolean useDelay, final MazeObject obj3) {
	...
	final String result = CommonDialogs.showInputDialog("Use which item?", "loopchute", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
			...
			LoopChute.getApplication().showMessage("That item has no more uses left.");
			...
			if (this.objectBeingUsed instanceof GenericBow) {
			    ...
			    LoopChute.getApplication().showMessage("Bow activated.");
			}
		    } else {
			LoopChute.getApplication().showMessage("Click to set target");
		    }
		    return;
		}
	    }
	...
	if (this.usingAnItem() && app.getMode() == Application.STATUS_GAME) {
	    final boolean visible = app.getMazeManager().getMaze().isSquareVisible(m.getPlayerLocationX(),
		...
		if ((target.isSolid() || !visible) && name.equals(new TeleportWand().getName())) {
		    this.setUsingAnItem(false);
		    LoopChute.getApplication().showMessage("Can't teleport there");
		}
		if (target.getName().equals(new Player().getName())) {
		    this.setUsingAnItem(false);
		    LoopChute.getApplication().showMessage("Don't aim at yourself!");
		}
		if (!target.isOfType(TypeConstants.TYPE_WALL) && name.equals(new WallBreakingWand().getName())) {
		    this.setUsingAnItem(false);
	...
	try {
	    ...
	    LoopChute.getApplication().showMessage(gameName2 + " on " + gameName1);
