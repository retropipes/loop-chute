...
class GameGUIManager {
	...
	if (this.vwMgr == null) {
	    this.vwMgr = LoopChute.getApplication().getGameManager().getViewManager();
	    this.setUpGUI();
	}
    }
	...
	if (this.outputFrame.isVisible()) {
		...
		for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
		    ...
		    try {
			if (visible) {
			    final MazeObject obj1 = app.getMazeManager().getMaze().getCell(y, x, m.getPlayerLocationZ(),
				    MazeConstants.LAYER_GROUND);
			    final MazeObject obj2 = app.getMazeManager().getMaze().getCell(y, x, m.getPlayerLocationZ(),
				    MazeConstants.LAYER_OBJECT);
			    final BufferedImageIcon img1 = GraphicsManager.getImage(
				    obj1.gameRenderHook(y, x, m.getPlayerLocationZ()), obj1.getGameBaseName(),
				    obj1.getGameTemplateColor(), obj1.getGameAttributeName(),
				    obj1.getGameAttributeTemplateColor());
			    final BufferedImageIcon img2 = GraphicsManager.getImage(
				    obj2.gameRenderHook(y, x, m.getPlayerLocationZ()), obj2.getGameBaseName(),
				    obj2.getGameTemplateColor(), obj2.getGameAttributeName(),
				...
				this.drawGrid[xFix][yFix]
					.setIcon(GraphicsManager.getVirtualCompositeImage(img1, img2, img3));
			    } else {
				this.drawGrid[xFix][yFix].setIcon(GraphicsManager.getCompositeImage(img1, img2));
			    }
			} else {
			    this.drawGrid[xFix][yFix].setIcon(GraphicsManager.getImage("Darkness", "Darkness",
				    ColorConstants.COLOR_NONE, "", ColorConstants.COLOR_NONE));
			}
		    } catch (final ArrayIndexOutOfBoundsException ae) {
			final EmptyVoid ev = new EmptyVoid();
			this.drawGrid[xFix][yFix]
				.setIcon(GraphicsManager.getImage(ev.gameRenderHook(y, x, m.getPlayerLocationZ()),
					ev.getGameBaseName(), ev.getGameTemplateColor(), ev.getGameAttributeName(),
	...
	if (this.outputFrame.isVisible()) {
	    ...
	    visible = app.getMazeManager().getMaze().isSquareVisible(m.getPlayerLocationX(), m.getPlayerLocationY(), x,
		    y);
	    try {
		if (visible) {
		    MazeObject obj1, obj2;
		    obj1 = m.getCell(x, y, m.getPlayerLocationZ(), MazeConstants.LAYER_GROUND);
		    obj2 = m.getCell(x, y, m.getPlayerLocationZ(), MazeConstants.LAYER_OBJECT);
		    final BufferedImageIcon img1 = GraphicsManager.getImage(
			    obj1.gameRenderHook(y, x, m.getPlayerLocationZ()), obj1.getGameBaseName(),
			    obj1.getGameTemplateColor(), obj1.getGameAttributeName(),
			    obj1.getGameAttributeTemplateColor());
	...
	public void handleMovement(final KeyEvent e) {
	    try {
		final GameLogicManager glm = LoopChute.getApplication().getGameManager();
