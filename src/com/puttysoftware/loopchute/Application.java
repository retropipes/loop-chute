...
import org.retropipes.diane.edge.EdgeGenerator;
...
public class Application {
	...
	final MazeObject[] groundTypes = this.objects.getAllGenerationEligibleTypedObjects();
	final String[] edgeNameSuffixes = EdgeGenerator.generateAllEdgedImageNameSuffixes();
	...
	for (int x = 0; x < groundTypes.length - 1; x++) {
		...
		final BufferedImageIcon[] edges = EdgeGenerator.generateAllEdgedImages(
			GraphicsManager.getImage(groundTypes[x].getName(), groundTypes[x].getBaseName(),
				groundTypes[x].getTemplateColor(), groundTypes[x].getAttributeName(),
				groundTypes[x].getAttributeTemplateColor()),
			GraphicsManager.getImage(groundTypes[y].getName(), groundTypes[y].getBaseName(),
				groundTypes[y].getTemplateColor(), groundTypes[y].getAttributeName(),
				groundTypes[y].getAttributeTemplateColor()),
			GraphicsManager.generateEdgeColor(groundTypes[x].getTemplateColor(),
				groundTypes[y].getTemplateColor()));
		for (int z = 0; z < edges.length; z++) {
		    GraphicsManager.addImageToCache(groundTypes[x].getName() + "/" + groundTypes[y].getName()
			    + " Transition " + edgeFriendlyNameSuffixes[z], edges[z]);
		    final GeneratedEdge ge = new GeneratedEdge(
			    groundTypes[x].getName() + "/" + groundTypes[y].getName() + " Transition "
				    + edgeFriendlyNameSuffixes[z],
			    groundTypes[x].getBaseName() + "_" + groundTypes[y].getBaseName() + edgeNameSuffixes[z],
